...
class ConfigView(qtw.QDialog):
    ...
    def _init_buttons(self) -> qtw.QWidget:
        ...
        sav_fun = lambda _, button=sav_but, checked=None: self._controller.store_to_file(button)
        sav_but.clicked.connect(sav_fun)
        but_lay.addWidget(sav_but)
        rst_but = qtw.QPushButton("Reset")
        rst_fun = lambda _, button=rst_but, checked=None: self._controller.reset(button)

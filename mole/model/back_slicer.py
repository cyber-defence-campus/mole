...
class MediumLevelILInstructionVisitor:
    ...
    def __init__(
            self,
            bv: bn.BinaryView,
            bn_expr_targets: Dict[str, Set[bn.MediumLevelILInstruction]],
            tag: str = "Visitor"
        ) -> None:
        """
        """
        self.bv = bv
        self.tag = tag
        self.bn_expr_targets = bn_expr_targets
        self.bn_expr_visited = {}
        return
    ...
    def get_instr_info(
        ...
        """
        This method returns a string with information about the expression `bn_expr`.
        """
    ...
    def visit_var_ssa_definition(
        ...
        """
        This method visits the expression defining variable `bn_var` within the function `bn_func`
        (backward slicing). If no defining expression is found within `bn_func`, it is checked
        whether variable `bn_var` belongs to a function argument. If so, backward slicing continues
        at the calling function.
        """
        bn_expr = bn_func.get_ssa_var_definition(bn_var)
        if bn_expr is None:
            vs = set()
            for param_num, param_var in enumerate(bn_func.source_function.parameter_vars):
                if param_var == bn_var.var:
                    ...
                    for code_ref in self.bv.get_code_refs(bn_func_addr):
                        ...
                        call = func.get_low_level_il_at(addr).mlil.ssa_form
                        parm = call.params[param_num]
                        vs.update(self.visit(parm, func.mlil.ssa_form))
            return vs
    ...
    def visit(
        ...
        """
        This method calls a dedicated visit function based on the operation name of expression
        `bn_expr`, belonging to function `bn_func`. Also it informs in case one of the targeted
        expressions is reached.
        """
        # Targeted expression reached
        for symbol_name, bn_expr_targets in self.bn_expr_targets.items():
            if bn_expr in bn_expr_targets:
                address = bn_expr.instr.address
                Logger.info(self.tag, f"0x{address:x} Source '{symbol_name:s}' (--> HIT <--)")
        # Expression visited before
        if bn_expr in self.bn_expr_visited:
            return self.bn_expr_visited[bn_expr]
        self.bn_expr_visited[bn_expr] = set()
        # Call dedicated visit functions
        o_name = bn_expr.operation.name.lower()
        f_name = f"visit_{o_name:s}"
        if hasattr(self, f_name):
            self.bn_expr_visited[bn_expr] = getattr(self, f_name)(bn_expr, bn_func)
            return self.bn_expr_visited[bn_expr]
...
class MediumLevelILVarSsaSlicer(MediumLevelILInstructionVisitor):
    """
    """
    ...
    def __init__(
        ...
        """
        """
        super().__init__(bv, bn_expr_targets, tag)
        return
    ...
    def visit_mlil_add(
        ...
        """
        This method visits the `MediumLevelILAdd` expression `bn_expr`, belonging to function
        `bn_func`.
        """
        Logger.debug(self.tag, self.get_instr_info(bn_expr) + " (MLIL_VAR_ADD)")
        vs = set()
        vs.update(self.visit(bn_expr.left, bn_func))
        vs.update(self.visit(bn_expr.right, bn_func))
        return vs
    ...
    def visit_mlil_address_of(
        ...
        """
        This method visits the  `MediumLevelILAddressOf` expression `bn_expr`, belonging to function
        `bn_func`.
        """
        Logger.debug(self.tag, self.get_instr_info(bn_expr) + " (MLIL_ADDRESS_OF)")
        return set()
    
    def visit_mlil_call_ssa(
            self,
            bn_expr: bn.MediumLevelILCallSsa,
            bn_func: bn.MediumLevelILFunction
        ) -> Set[bn.SSAVariable]:
        """
        This method visits the `MediumLevelILCallSsa` expression `bn_expr`, belonging to function
        `bn_func`.
        """
        Logger.debug(self.tag, self.get_instr_info(bn_expr) + " (MLIL_CALL_SSA)")
        vs = set()
        for output in bn_expr.output:
            vs.add(output)
        for param in bn_expr.params:
            vs.update(self.visit(param, bn_func))
        return vs
    ...
    def visit_mlil_const(
        ...
        """
        This method visits the `MediumLevelILConst` expression `bn_expr`, belonging to function
        `bn_func`.
        """
    ...
    def visit_mlil_const_ptr(
        ...
        """
        This method visits the `MediumLevelILConstPtr` expression `bn_expr`, belonging to function
        `bn_func`.
        """
        Logger.debug(self.tag, self.get_instr_info(bn_expr) + " (MLIL_CONST_PTR)")
        return set()
    
    def visit_mlil_load_ssa(
            self,
            bn_expr: bn.MediumLevelILLoadSsa,
            bn_func: bn.MediumLevelILFunction
        ) -> Set[bn.SSAVariable]:
        """
        This method visits the  `MediumLevelILLoadSsa` expression `bn_expr`, belonging to function
        `bn_func`.
        """
        Logger.debug(self.tag, self.get_instr_info(bn_expr) + " (MLIL_LOAD_SSA)")
        vs = set()
    ...
    def visit_mlil_lsl(
        ...
        """
        This method visits the `MediumLevelILLsl` expression `bn_expr`, belonging to function
        `bn_func`.
        """
        Logger.debug(self.tag, self.get_instr_info(bn_expr) + " (MLIL_VAR_LSL)")
        vs = set()
        vs.update(self.visit(bn_expr.left, bn_func))
        vs.update(self.visit(bn_expr.right, bn_func))
        return vs
    ...
    def visit_mlil_lsr(
        ...
        """
        This method visits the `MediumLevelILLsr` expression `bn_expr`, belonging to function
        `bn_func`.
        """
        Logger.debug(self.tag, self.get_instr_info(bn_expr) + " (MLIL_VAR_LSR)")
        vs = set()
        vs.update(self.visit(bn_expr.left, bn_func))
        vs.update(self.visit(bn_expr.right, bn_func))
        return vs
    ...
    def visit_mlil_set_var_ssa(
            self,
            bn_expr: bn.MediumLevelILSetVarSsa,
            bn_func: bn.MediumLevelILFunction
        ) -> Set[bn.SSAVariable]:
        """
        This method visits the  `MediumLevelILSetVarSsa` expression `bn_expr`, belonging to function
        `bn_func`.
        """
    ...
    def visit_mlil_sub(
        ...
        """
        This method visits the `MediumLevelILSub` expression `bn_expr`, belonging to function
        `bn_func`.
        """
        Logger.debug(self.tag, self.get_instr_info(bn_expr) + " (MLIL_VAR_SUB)")
        vs = set()
        vs.update(self.visit(bn_expr.left, bn_func))
        vs.update(self.visit(bn_expr.right, bn_func))
        return vs
    
    def visit_mlil_var_aliased(
            self,
            bn_expr: bn.MediumLevelILVarAliased,
            bn_func: bn.MediumLevelILFunction
        ) -> Set[bn.SSAVariable]:
        """
        This method visits the `MediumLevelILVarAliased` expression `bn_expr`, belonging to function
        `bn_func`.
        """
        Logger.debug(self.tag, self.get_instr_info(bn_expr) + " (MLIL_VAR_ALIASED)")
    ...
    def visit_mlil_var_phi(
        ...
        """
        This method visits the `MediumLevelILVarPhi` expression `bn_expr`, belonging to function
        `bn_func`.
        """
    ...
    def visit_mlil_var_ssa(
        ...
        """
        This method visits the `MediumLevelILVarSsa` expression `bn_expr`, belonging to function
        `bn_func`.
        """
    ...
    def slice_backwards(
        ...
        variables that influence the expression `bn_expr`.
        """
        return self.visit(bn_expr, bn_expr.function)


# class OldMediumLevelILInstructionVisitor:
#     """
#     Base class to visit `MediumLevelILInstruction` expressions.
#     """
...
# class OldMediumLevelILVarSsaModeler(OldMediumLevelILInstructionVisitor):
#     """

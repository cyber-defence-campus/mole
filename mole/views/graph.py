...
class Node(qtw.QGraphicsObject):
    ...
    def _update_rect(self) -> None:
        ...
        self._rect = qtc.QRectF(
            0, 0, text_width + 2 * self._padding, text_height + 2 * self._padding
        )
        return
    ...
    def paint(
        ...
        painter.drawText(
            self.boundingRect(), qtc.Qt.AlignmentFlag.AlignCenter, self._name
        )
        return
...
class Edge(qtw.QGraphicsItem):
    ...
    def _draw_arrow(
        self, painter: qtui.QPainter, start: qtc.QPointF, end: qtc.QPointF
    ) -> None:
        ...
        painter.setBrush(
            qtui.QBrush(
                self._get_node_color(
                    self._source._node_backing, self._dest._node_backing
                )
            )
        )
    ...
    def _arrow_target(self) -> qtc.QPointF:
        ...
        target = qtc.QPointF(
            center.x() + (normal.x() * rect.width() / 2),
            center.y() + (normal.y() * rect.height() / 2),
        )
        return target

    def paint(
        self, painter: qtui.QPainter, option: qtw.QStyleOptionGraphicsItem, widget=None
    ) -> None:
        """Override from qtw.QGraphicsItem
...
class GraphView(qtw.QGraphicsView):
    ...
    def get_node_color(
        self, src_node: bn.MediumLevelILFunction, dest_node: bn.MediumLevelILFunction
    ) -> qtui.QColor:
        # warm, golden yellow is the default
        highlight_color = qtui.QColor("#FFD166")
        if (
            "snk" in self._graph.nodes[src_node]
            or "snk" in self._graph.nodes[dest_node]
        ):
            # muted, earthy red
            highlight_color = qtui.QColor("#D65A5A")
        if (
            "src" in self._graph.nodes[dest_node]
            or "src" in self._graph.nodes[dest_node]
        ):
            # soft, warm red
            highlight_color = qtui.QColor("#FF9999")

        if (
            self._graph.nodes[src_node]["in_path"]
            and self._graph.nodes[dest_node]["in_path"]
        ):
    ...
    def load_graph(
        ...
        for node in self._graph:
            if not show_all_nodes and not self._graph.nodes[node]["in_path"]:
                continue
            item = Node(
                node, self.get_node_text, self.on_click_callback, self.get_node_color
            )
    ...
    def layout(self) -> None:
        positions = nx.multipartite_layout(
            self._graph, subset_key="call_level", align="horizontal"
        )
        ...
        max_width = (
            max(
                sum(self._nodes_map[node].boundingRect().width() for node in nodes)
                + (len(nodes) - 1) * 20.0
        ...
        for level, nodes in levels_nodes.items():
            ...
            total_width = sum(
                self._nodes_map[node].boundingRect().width() for node in nodes
            )
            spacing = (
                (max_width - total_width) / (node_count - 1) if node_count > 1 else 0
            )
        ...
        for node, (ox, oy) in positions.items():
            ...
            animation.setEndValue(
                qtc.QPointF(new_x_positions[node], oy * vertical_spacing)
            )
...
class GraphWidget(qtw.QWidget):
    ...
    def load_path(self, bv: bn.BinaryView, path: Path, path_id: int) -> None:
        ...
        self.view.load_graph(
            bv, path, path_id, not self._show_in_path_checkbox.isChecked()
        )
        return

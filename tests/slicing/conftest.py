...
class SlicingTestBase:
    """
    This class implements shared functionality for slicing tests.
    """
    ...
    def load_files(self, names: List[str]) -> List[str]:
        """
        This method returns all files in the bin directory matching `name` but ignoring the
        file extension.
        """
        # Navigate from tests/slicing to tests/data/bin
        test_slicing_dir = os.path.dirname(__file__)
        tests_dir = os.path.dirname(test_slicing_dir)
        directory = os.path.join(tests_dir, "data", "bin")
        files = []

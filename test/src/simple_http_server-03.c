...
__attribute__ ((noinline)) 
void log_request(const char *method, const char *buffer);

__attribute__ ((noinline)) 
void process_request(int client_socket, const char *method, const char *buffer);

__attribute__ ((noinline)) 
void handle_put_request(int client_socket);

__attribute__ ((noinline)) 
void handle_delete_request(int client_socket);

__attribute__ ((noinline)) 
void wrap_and_execute(const char *buffer);
...
void handle_client(int client_socket) {
    ...
    } else if (strncmp(method, "PUT ", 4) == 0) {
        handle_put_request(client_socket);
...
void handle_get_request(int client_socket) {
    ...
    log_request("GET", buffer);
    process_request(client_socket, "GET", buffer);
    free(buffer);
}
...
void handle_post_request(int client_socket) {
    ...
    log_request("POST", buffer);
    process_request(client_socket, "POST", buffer);
    free(buffer);
}
...
void handle_put_request(int client_socket) {
    ...
    log_request("PUT", buffer);
    process_request(client_socket, "PUT", buffer);
    free(buffer);
}
...
void handle_delete_request(int client_socket) {
    ...
    log_request("DELETE", buffer);
    process_request(client_socket, "DELETE", buffer);
    free(buffer);
}

void log_request(const char *method, const char *buffer) {
    printf("Received %s request: %s\n", method, buffer);
}

void process_request(int client_socket, const char *method, const char *buffer) {
    if (strcmp(method, "GET") == 0 || strcmp(method, "POST") == 0) {
        wrap_and_execute(buffer);
